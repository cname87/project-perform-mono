...
async function setupDatabase(obj: IObjects) {
  ...
  try {
    database.configDatabase(obj.config, obj.logger, obj.dumpError);
    obj.dbConnection = await database.connectToDB();
    return;
  } catch (err) {
...
async function runApp() {
  ...
  if (obj.dbConnection.readyState === 1) {
    ...
    /* create an express-session object for session state persistence */
    /* need mongoStore so we can close the connection */
    obj.mongoStore = await database.createStore();
    obj.sessionStore = database.createSession(
      obj.mongoStore,
      config.COOKIE_KEY,
    );
  } else {
