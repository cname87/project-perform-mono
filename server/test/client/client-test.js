 ...
 * config.HANG_CLEAR_PROD = 5000; i.e. set up hang-test.
 */
...
describe('Server', () => {
  ...
  const stubProcessEmit = sinon.stub(process, 'emit');
  /* stub process.exit */
  ...
  before('before', async () => {
    ...
    spyHangDebug = sinon.spy(
      index.appObjects.config.HANG_TEST_CONTROLLER,
      'debug',
    );
  ...
  it('Test client-fired server functionality', async () => {
    ...
    /* set false if 'Hang test start' event received,
     * then true if 'Slow process cleared' received
     * (which signals end of the hang test) */
    let hangTestCleared = true;
    await new Promise((resolve, reject) => {
      const browserEventscallback = (arg) => {
        ...
        try {
          switch (arg.message) {
            ...
            case 'Check server up':
              break;
            case 'Hang test start':
              hangTestCleared = false;
              sinon.resetHistory();
              break;
            case 'Hang test end':
            case '404 test end':
        ...
        /* only close when browser tests complete and
         * hang slow process cleared (or hang test never called) */
        if (endTestCalled && hangTestCleared) {
          ...
          setTimeout( () =>
            ...
            /* unref() doesn't work */
            browserInstance.kill(), 5000);
          resolve();
        }
      };

      const slowProcessCallback = (arg) => {
        switch (arg.message) {
          case 'Slow process cleared':
            expect(spyHangDebug.lastCall.lastArg).to.eql(
              '\\hang.js: slow process ended',
            );
            hangTestCleared = true;
            break;
        }
        /* only close when browser tests complete and
         * hang slow process cleared / never called) */
        if (endTestCalled && hangTestCleared) {
          resolve();
        }
      };
      ...
      /* slow process that runs after hang timeout caught here */
      eventEmitter.once('hangHang', slowProcessCallback);

      /* start chrome on mocha test page */
      browserInstance = spawn('C:\\Program Files (x86)\\Google\\Chrome\\Application\\chrome.exe',
        ['https://localhost:1337/client/test/',
