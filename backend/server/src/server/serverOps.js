...
const http_shutdown_1 = tslib_1.__importDefault(require("http-shutdown"));
/**
 ...
 */
function setupServer(serverType, serverOptions, app) {
    ...
    this.expressServer = http_shutdown_1.default(this.expressServer);
 ...
 * no error).
 */
async function listenServer(serverPort, listenTries = 3, listenTimeout = 5) {
    debug(modulename + ': running listenServer');
    async function listenCallback(resolve, reject) {
        ...
        function listenHandler() {
            ...
            debug(`${modulename}: ${this.name} server` +
        ...
        async function errorHandler(err) {
            ...
            if (err.code === 'EADDRINUSE') {
                ...
                if (this.listenErrors < listenTries) {
                    this.logger.error(modulename +
                        ': Port ' +
                        `${serverPort} is in use following attempt ` +
                        `${this.listenErrors} of ` +
                        `${listenTries} - retrying in ${listenTimeout}s`);
                    setTimeout(listenCallback.bind(this), listenTimeout * 1000, resolve, reject);
                }
                else {
                ...
                this.logger.error(modulename + ': Server listen error other than EADDRINUSE');
                reject(err);
            }
        }
...
async function stopServer() {
    ...
    function shutServer(resolve, reject) {
        this.expressServer.shutdown((err) => {
            if (err) {
 ...
 */
function configServer(name = '', logger = { error: console.error }, dumpError = console.error) {
    this.name = name;
    this.logger = logger;
    this.dumpError = dumpError;
}
//# sourceMappingURL=serverOps.js.map