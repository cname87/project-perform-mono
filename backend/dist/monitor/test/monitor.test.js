...
sinon_1.default.assert.expose(chai.assert, {
    prefix: '',
});
/* internal dependencies */
const indexPath = '../src/index';
const configMonitor_1 = require("../src/configMonitor");
describe('server monitor', () => {
    ...
    const checkMonitorDebugOnce = (spyDebugLog, spyDebugString, checkDebugCount) => {
    ...
    const checkMonitorDebug = async (spyDebugLog, spyDebugString) => {
        for (let checkDebugCount = 1; checkDebugCount <= 20; checkDebugCount++) {
            const response = await checkMonitorDebugOnce(spyDebugLog, spyDebugString, checkDebugCount);
    ...
    before('set up spies', async () => {
        ...
        configMonitor_1.config.WATCH_FILES = false;
        /* set maximum number of tries = 2 */
        configMonitor_1.config.MAX_STARTS = 2;
        ...
        monitorConfig.IS_MONITOR_DEBUG = configMonitor_1.config.IS_MONITOR_DEBUG;
        monitorConfig.MAX_STARTS = configMonitor_1.config.MAX_STARTS;
        /* spy on monitor functions */
        spyDebug = sinon_1.default.spy(monitor, 'debug');
        spyLoggerError = sinon_1.default.spy(monitor.logger, 'error');
        spyDumpError = sinon_1.default.spy(monitor, 'dumpError');
        /* spy on console.error (as node may send warnings there) */
        spyConsoleError = sinon_1.default.spy(console, 'error');
        /* confirm monitor is up */
        let spyDebugString = '\\index.js: Monitor received confirmation: Server is running';
    ...
    it('tests watch:restart', async () => {
        ...
        expect(spyDebug.calledWith('\\index.js: ' + 'forever restarting script because test changed')).to.be.true;
        /* check no errors */
    ...
    it('tests child restart, and not restart', async () => {
        ...
        expect(spyDumpError.notCalled).to.be.true;
        expect(spyDebug.calledWith(`\\index.js: forever restarting script - restart number: 1 of ${maxRetries}`)).to.be.true;
        /* send a second crash */
        child.send({
            action: 'crash',
        });
    ...
    it('tests uncaught exception', async () => {
        ...
        const stubProcess = sinon_1.default.stub(process, 'exit');
        /* allow monitor.js to load */
        await start();
        let spyDebugString = '\\index.js: Monitor received confirmation: Server is running';
        await checkMonitorDebug(spyDebug, spyDebugString);
    ...
    it('tests uncaught rejection', async () => {
        ...
        expect(stubProcess.calledWith(1), 'process.exit(1) called').to.be.true;
        stubProcess.restore();
        expect(spyLoggerError.calledWith(sinon_1.default.match('unhandled promise rejection'))).to.be.true;
        expect(spyConsoleError.notCalled).to.be.true;
        /* test and ensure monitor has exited */
        spyDebugString = "\\index.js: child exited with code '31017'";
        await checkMonitorDebug(spyDebug, spyDebugString);
    ...
    it('tests unexpected exit with no child', async () => {
        ...
        expect(stubProcess.calledWith(1), 'process.exit(1) called').to.be.true;
        stubProcess.restore();
        expect(spyLoggerError.calledWith('\\index.js: exiting due to: ' + 'Test - child not running')).to.be.true;
        /* close monitor */
    ...
    it('tests watch exit function path', async () => {
        ...
        configMonitor_1.config.WATCH_FILES = true;
